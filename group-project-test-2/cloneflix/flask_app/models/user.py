...
#method to save user's information to db 
    ...
    def save(cls, data):
        ...
        query = "INSERT INTO users (first_name, last_name, email, password, card_number, plan) VALUES (%(first_name)s, %(last_name)s, %(email)s, %(password)s, %(card_number)s, %(plan)s)"
    ...
    @classmethod
    def update(cls, data):
        query = "UPDATE users SET first_name = %(first_name)s, last_name = %(last_name)s, email = %(email)s , plan = %(plan)s, card_number = %(card_number)s WHERE id = %(id)s;"
